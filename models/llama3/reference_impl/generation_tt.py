...
import sys
spec = importlib.util.spec_from_file_location("models.demos.llama3.tt.model_config", "/home/cglagovich/tt-metal/models/demos/llama3/tt/model_config.py")
model_config = importlib.util.module_from_spec(spec)
sys.modules["models.demos.llama3.tt.model_config"] = model_config
spec.loader.exec_module(model_config)
# model_config = importlib.import_module("models.demos.llama3.tt.model_config")
TtModelArgs = model_config.TtModelArgs
...
class Llama:
    ...
    def build(
        ...
        if model_args.vision_chunk_size > 0:
            from .multimodal.model_tt import CrossAttentionTransformer
        ...
        # checkpoint_partial = {k: v for k, v in checkpoint.items() if k.startswith(text_prefix)}
        # model.load_state_dict(checkpoint_partial, strict=True)
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            print(f'Generating token: {cur_pos}/{total_len}')
            if is_vision:
