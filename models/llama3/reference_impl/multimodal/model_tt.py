...
from models.demos.llama3.tt.multimodal.llama_cross_attention_transformer_text import TtLlamaCrossAttentionTransformerText
...
class CrossAttentionTransformer(torch.nn.Module):
    ...
    def compute_vision_tokens_masks(
        ...
        if skip_vision_encoder:
            ...
            vision_tokens = vision_tokens[0].reshape(bsz, max_num_images, self.max_num_chunks, -1, self.model_dim).float()
        ...
        vision_tokens_squeeze = torch.nn.functional.pad(vision_tokens_squeeze, (0, 0, 0, 4224-vision_tokens_squeeze.shape[2]), "constant", 0)
        ...
        cross_attention_masks = torch.nn.functional.pad(cross_attention_masks, (0, 4224-cross_attention_masks.shape[3]), "constant", get_negative_inf_value(torch.float32))
    ...
    def forward(
        ...
        if mode == "prefill":
            xattn_mask_expand = torch.nn.functional.pad(xattn_mask_expand, (0, 0, 0, 128-xattn_mask_expand.shape[2]), "constant", get_negative_inf_value(torch.float32))
        ...
        if mode == "prefill":
            full_text_mask = torch.nn.functional.pad(full_text_mask, (0, 0, 0, 128-full_text_mask.shape[2]), "constant", 0)
        full_text_mask_expand_1NSH = full_text_mask.expand(-1, self.configuration.n_heads // self.configuration.num_devices, -1, self.configuration.head_dim)
        ...
        if mode == "prefill":
            ...
            rot_mats = get_prefill_rot_mat(self.configuration.head_dim, self.configuration.max_seq_len, self.mesh_device, seq_len=seq_len)
            ...
            tt_xattn_mask = ttnn.reshape(
                tt_xattn_mask,
                shape=ttnn.Shape(
                    [batch, self.configuration.n_heads // self.configuration.num_devices, seq_len, xattn_mask.shape[-1]],
            ...
            tt_full_text_mask_expand_1NSH = ttnn.reshape(
                ...
                shape=ttnn.Shape(
                    [batch, self.configuration.n_heads // self.configuration.num_devices, seq_len, self.configuration.head_dim],
                    [batch, self.configuration.n_heads // self.configuration.num_devices, 32, self.configuration.head_dim],
                ),
            )
